...
- **Version:** 1.2.6 — packaged at `release/pdf-mana-1.2.6.zip` (older zips in `release/archive/`). **Versioning is odometer-style since 1.2.1**: each component counts 0–9 and carries (1.2.9 → 1.3.0 → … → 1.9.9 → 2.0.0); `npm run release` implements the carry. **v1.2.6 is published as the latest GitHub Release** (2026-07-11, zip attached); v1.0.19–v1.2.5 are deliberately tag-only (all tags are pushed).
