...
const mkPages = (n: number, docId = 'd1'): PageDescriptor[] =>
  Array.from({ length: n }, (_, i) => ({ id: `p${i}`, docId, pageIndex: i, rotation: 0 }));
...
describe('persist', () => {
  it('round-trips a saved session, bytes intact', async () => {
    ...
    await saveState({ savedAt: 123, pages: mkPages(3), splitMarks: ['p1'], look: 'bubble' });
  ...
  it('returns null when a referenced doc is missing its bytes', async () => {
    ...
    await saveState({ savedAt: 1, pages: mkPages(2, 'd1'), splitMarks: [], look: 'blocks' });
    expect(await loadSession()).toBeNull();
  });

  it('returns only the docs referenced by the saved pages', async () => {
    await saveDocs([mkDoc('d1', 'a', [1]), mkDoc('d2', 'b', [2])]);
    await saveState({ savedAt: 1, pages: mkPages(1, 'd1'), splitMarks: [], look: 'blocks' });
    const s = await loadSession();
  ...
  it('saveState overwrites the previous session', async () => {
    ...
    await saveState({ savedAt: 1, pages: mkPages(3, 'd1'), splitMarks: [], look: 'blocks' });
    await saveState({ savedAt: 2, pages: mkPages(1, 'd1'), splitMarks: [], look: 'sunny' });
    const s = await loadSession();
